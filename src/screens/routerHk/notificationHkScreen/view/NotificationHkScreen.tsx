...
import { ContentNotificationHk } from '../modal/notificationModel';
import { useNotificationHkViewModel } from '../viewmodal/useNotificationHkViewModel';
import ItemNotificationHk from './component/ItemNotificationHk';
...
import NotificationSkeletonHk from './component/NotificationSkeleton';

type Props = NativeStackScreenProps<MainParams, 'NotificationHkScreen'>;
const totalNotification = 100; // Hoặc truyền từ props nếu dynamic
const ICON_SECTION_WIDTH = s(130);

// eslint-disable-next-line @typescript-eslint/no-unused-vars
const NotificationHkScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const {
    ...
    onDetail,
    readAll,
  } = useNotificationHkViewModel();

  // ─── Refs & shared values để show/hide nút cuộn ─────────────────────

  const flatListRef = useRef<FlashList<ContentNotificationHk> | null>(null);
  ...
  const renderItem = ({ item }: { item: ContentNotificationHk }) => (
    <ItemNotificationHk
      item={item}
      onDetail={() => {
        if (!item.read) {
        ...
        // navigate('DetailRoomScreen', { id: item.id.toString() });
      }}
      toggleRead={() => {
        ...
        {isLoading && flatData.length === 0 ? (
          <View style={styles.listContentSkeleton}>
            {new Array(6).fill(0).map((_, index) => (
              <NotificationSkeletonHk key={index} />
            ))}
          </View>
        ) : (
          <FlashList
...
export default NotificationHkScreen;
