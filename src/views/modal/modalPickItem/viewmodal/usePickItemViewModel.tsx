...
import { fetchPickItemData, IPickItem } from '../modal/PickItemModal';

const ITEMS_PER_PAGE = 50;
const DEBOUNCE_DELAY = 300;

export function usePickItemViewModel() {
  ...
  const {
    ...
    hasNextPage,
    isRefetching,
  } = useInfiniteQuery<IPickItem[], Error>({
    queryKey: ['listPickItem', searchKey.trim(), searchKey],
    queryFn: async ({ pageParam }: { pageParam?: unknown }) => {
      const page = typeof pageParam === 'number' ? pageParam : 1;
      return fetchPickItemData(page, ITEMS_PER_PAGE, searchKey);
    },
