...
import { fetchPickLocalData, IPickLocal } from '../modal/PickLocalModal';

const ITEMS_PER_PAGE = 50;
const DEBOUNCE_DELAY = 300;

export function usePickLocalViewModel() {
  ...
  const {
    ...
    hasNextPage,
    isRefetching,
  } = useInfiniteQuery<IPickLocal[], Error>({
    queryKey: ['listPickLocal', searchKey.trim(), searchKey],
    queryFn: async ({ pageParam }: { pageParam?: unknown }) => {
      const page = typeof pageParam === 'number' ? pageParam : 1;
      return fetchPickLocalData(page, ITEMS_PER_PAGE, searchKey);
    },
